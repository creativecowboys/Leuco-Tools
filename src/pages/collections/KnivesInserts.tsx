...
function CollectionPage({ title, subtitle, description, collectionHandle }: { title: string; subtitle: string; description: string; collectionHandle: string }) {
    ...
    return (
        <div>
            ...
            <div id="products-grid" className="py-20 px-4 md:px-12 max-w-[1440px] mx-auto">
                <CollectionGrid products={products} loading={loading} title={title} />
            </div>
        </div>
    );
}
...
export default function KnivesInserts() {
    return <CollectionPage title="Knives & Inserts" subtitle="CUTTING EDGES" description="Carbide and HSS knives and inserts for all LEUCO tooling systems. Maintain peak performance with genuine LEUCO replacement edges." collectionHandle="knives-and-inserts" />;
}
